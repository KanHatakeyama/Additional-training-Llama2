...
base_path = "res/1021ig_with_large_noise_mult_lang/"
r = 1024
lr = 0.001
lora_alpha = r
total_epochs = 10
...
context_path = "../smallDB/1018ig/context_many_mult_lang_plus_oa.json"
...
max_context = 8
i_list = np.logspace(1, 3.7, 20)
lit_list = range(1, max_context+1)
i_list = [int(i) for i in i_list]

for l in lit_list:
    for i in i_list:
        key = f"{l}_{i}"
        value = context_list[:l]
        value += context_list[max_context:max_context+i]
...
r_list = [32, 64, 128, 256, 512, 1024, 2048]
random.shuffle(r_list)
lora_alpha_list = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048]
random.shuffle(lora_alpha_list)
        ...
        for lora_alpha in lora_alpha_list:
            for cond in cond_list:
                ...
                for i in range(total_epochs):
                    epoch += 1
                    eval_path = base_path + \
                        f"{lora_key}_{epoch}_{r}_{lora_alpha}_{cond}.csv"
                    print(eval_path)
